 ...
 *
 * A intensidade do bloco é a quantidade: a janela com 89 contratos é
 * sólida, a com 1 é fantasma. Não é escala medida — é ordem de
 * grandeza, e o número exato continua na dica e na lista ao lado.
 */
export function FaixaPeriodos({ periodos }: {
  ...
  const comHora = periodos.flatMap(p => {
    ...
    if (a === null) return []
    return [{ janela: p.janela, qtd: p.qtd, a, b: emMinutos(p.janela.split(/[-–]/)[1]) }]
  })
  ...
  const maior = Math.max(1, ...comHora.map(p => p.qtd))
      ...
      {comHora.length > 0 && (
        <>
          <div className="faixa-dia"
            title={comHora.map(p => `${p.janela} (${p.qtd})`).join('\n')}
            aria-hidden>
            {comHora.map(p => {
              const esq = pct(p.a)
              ...
              return (
                <div key={p.janela} className="bloco"
                  style={{
                    ...
                    ['--bloco-cor' as string]:
                      `color-mix(in srgb, var(--st-execucao) ${
                        Math.round(30 + 65 * (p.qtd / maior))}%, transparent)`,
                  }} />
              )
            })}
          </div>
      ...
      <div className="mt-0.5 flex flex-wrap gap-x-2 gap-y-0.5 text-[10px] text-graf-400">
        {comHora.slice(0, 4).map(p => (
          <span key={p.janela} className="tabular whitespace-nowrap">
            ...
            <span className="ml-0.5 text-graf-600">{p.qtd}</span>
          </span>
        ))}
        {comHora.length > 4 && (
